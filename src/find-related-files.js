  ...
  const glimmer = group
    .map(item => {
      if (item.endsWith('-js')) {
        ...
        // Templates have -gts/-gjs but not -ts
        return [
          item.replace('-hbs', '-gts'),
          item.replace('-hbs', '-gjs')
        ]
      } else {
        return []
      }
    ...
    .flat()
  return group.concat(typescript).concat(glimmer)
})
...
const types = [
  ...
  { module: 'component-unit', exp: /^()tests\/unit\/components\/(.+)-test\.(js|ts|gts)$/ },
  { module: 'component-integration', exp: /^()tests\/integration\/components\/(.+)-test\.(js|ts|gts)$/ },
  ...
  { module: 'controller-unit', exp: /^()tests\/unit\/controllers\/(.+)-test\.(js|ts|gts)$/ },
  { module: 'controller-integration', exp: /^()tests\/integration\/controllers\/(.+)-test\.(js|ts|gts)$/ },
  { module: 'controller-template', exp: /^(app|addon|lib\/(?:.+)\/addon)\/templates\/(.+)\.(hbs|gts|gjs)$/ },
    ...
    switch (subtype) {
      ...
      case 'template':
        if (type === 'controller') {
          ...
          return `${hostType}/components/${part}.${ext}`
        } else {
...
function typeKeyToLabel (typeKey) {
  // Normalize the typeKey to handle -gts and -gjs variants
  // Templates should normalize to -hbs, others to -ts
  let normalizedKey = typeKey
  if (normalizedKey.includes('template')) {
    normalizedKey = normalizedKey.replace(/-gts$/, '-hbs').replace(/-gjs$/, '-hbs')
  } else {
    normalizedKey = normalizedKey.replace(/-gts$/, '-ts').replace(/-gjs$/, '-ts')
  }
