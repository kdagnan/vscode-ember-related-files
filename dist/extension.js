...
// src/find-related-files.js
var require_find_related_files = __commonJS({
  "src/find-related-files.js"(exports2, module2) {
    "use babel";
    var fs2 = require("fs");
    var path2 = require("path");
    ...
    function detectHostType(rootPath) {
      ...
      if (!HOST_TYPE_CACHE[hostPath]) {
        HOST_TYPE_CACHE[hostPath] = fs2.existsSync(join2(String(rootPath), "addon")) ? "addon" : "app";
      }
    ...
    function getRelatedTypeKeys(typeKey) {
      return groups.find((group) => group.indexOf(typeKey) !== -1).filter((key) => key !== typeKey);
    }
    ...
    function pathToLabel(typeKey, filePath) {
      const typeDef = types.find(({ module: module3 }) => module3 === typeKey);
      if (typeDef) {
    ...
    function findRelatedFiles2(rootPath, filePath) {
      const type = detectType(rootPath, filePath);
      if (!type) {
        return [];
      }
      return getRelatedTypeKeys(type.key).map((typeKey) => ({
        label: typeKeyToLabel(typeKey),
        path: getPath(type, typeKey)
      })).filter(({ path: filePath2 }) => fs2.existsSync(path2.join(rootPath, filePath2)));
    }
    ...
    module2.exports = (...args) => {
      ...
      return findRelatedFiles2(...args);
    };
    module2.exports.findRelatedFiles = findRelatedFiles2;
    module2.exports.findType = findType;
    module2.exports.getRelatedTypeKeys = getRelatedTypeKeys;
    module2.exports.detectType = detectType;
    module2.exports.getPath = getPath;
  }
});

// src/extension.js
var { window, workspace, commands, Uri } = require("vscode");
var fs = require("fs");
var path = require("path");
var { join, sep: fileSeparator } = require("path");
var findRelated = require_find_related_files();
var findRelatedFiles = findRelated.findRelatedFiles;
var TypeItem = class {
...
function open(item) {
  workspace.openTextDocument(item.uri()).then(
    (doc) => window.showTextDocument(doc.uri, { preview: config("enablePreview") })
  );
}
function config(key) {
...
function activate(context) {
  const disposable = commands.registerCommand("relatedFiles.relatedFiles", () => {
    ...
    const items = findRelatedFiles(normalizedRootPath, normalizedFilePath).map((item) => new TypeItem(item, normalizedRootPath));
    if (items.length === 0) {
      return;
    }
